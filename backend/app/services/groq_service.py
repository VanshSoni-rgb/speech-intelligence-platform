        ...
        try:
            ...
            for chunk_idx, (chunk_path, time_offset) in enumerate(chunks):
                with open(chunk_path, "rb") as file_obj:
                    extra_args = {}
                    # Use explicit language or carry over detected language from previous chunks
                    raw_current_lang = primary_language or (detected_lang.lower() if chunk_idx > 0 and detected_lang else None)
                    if raw_current_lang:
                        clean_raw = raw_current_lang.strip().lower()
                        norm_lang = lang_map.get(clean_raw, clean_raw)
                        if len(norm_lang) <= 3:
                            extra_args["language"] = norm_lang
